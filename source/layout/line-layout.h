...
namespace plutobook {
    ...
    public:
        ...
        LineItem(Type type, Box* box, uint32_t startOffset, uint32_t endOffset)
            : m_box(box), m_startOffset(startOffset), m_endOffset(endOffset),
              m_type(type) {}
        ...
        void setCollapseType(CollapseType collapseType) {
        ...
        void setHasCollapsibleNewline(bool hasNewline) {
            m_hasCollapsibleNewline = hasNewline;
        }

        bool isTextItem() const {
            return m_type == Type::NormalText ||
                   m_type == Type::TabulationText || m_type == Type::LeaderText;
        }
        bool isBreakOpportunity() const {
            return m_type == Type::SoftBreakOpportunity ||
                   m_type == Type::HardBreakOpportunity;
        }
    ...
    using LineItems = std::vector<LineItem>;
    ...
    protected:
        ...
        void enterBidi(Box* box, Direction direction, UChar enterLtr,
                       UChar enterRtl, UChar exit);
        void exitBidi(Box* box);

        LineItem& appendItem(LineItem::Type type, Box* box, uint32_t start,
                             uint32_t end);
        LineItem& appendItem(LineItem::Type type, Box* box, UChar character);
        LineItem& appendOpaqueItem(LineItem::Type type, Box* box,
                                   UChar character);
        LineItem& appendOpaqueItem(LineItem::Type type, Box* box);
        LineItem& appendTextItem(LineItem::Type type, Box* box,
                                 const UString& text);
        ...
        bool shouldInsertBreakOpportunityAfterLeadingPreservedSpaces(
            Box* box, const UString& text, int start) const;
        int insertBreakOpportunityAfterLeadingPreservedSpaces(
            Box* box, const UString& text, int start);
    ...
    struct LineItemRun {
        LineItemRun(const LineItem& item, uint32_t index, uint32_t start,
                    uint32_t end)
            : item(&item), itemIndex(index), startOffset(start),
              endOffset(end) {}
    ...
    public:
        ...
        bool canBreakAfterLastRun() const {
            return !m_runs.empty() && m_runs.back().canBreakAfter;
        }
    ...
    public:
        ...
        static void reorderVisual(const std::vector<UBiDiLevel>& levels,
    ...
    class LineBreaker {
    public:
        LineBreaker(BlockFlowBox* block, FragmentBuilder* fragmentainer,
                    LineItemsData& data);
        ~LineBreaker();
    ...
    private:
        enum class LineBreakState { Continue, Trailing, Done };

        LineItemRun& addItemRun(const LineItem& item, uint32_t startOffset,
                                uint32_t endOffset);
        LineItemRun& addItemRun(const LineItem& item) {
            return addItemRun(item, m_textOffset, item.endOffset());
        }
        ...
        void breakText(LineItemRun& run, const LineItem& item,
                       const RefPtr<TextShape>& shape, float availableWidth);
        void handleTrailingSpaces(const LineItem& item,
                                  const RefPtr<TextShape>& shape);

        void rewindOverflow(uint32_t newSize);
        void handleOverflow();

        float availableWidthToFit() const {
            return m_availableWidth + kLineLayoutEpsilon;
        }
        float remainingAvailableWidth() const {
            return availableWidthToFit() - m_currentWidth;
        }

        bool canFitOnLine() const {
            return m_currentWidth <= availableWidthToFit();
        }
        bool canFitOnLine(float extra) const {
            return extra + m_currentWidth <= availableWidthToFit();
        }
        bool canBreakAfter(const LineItemRun& run) const {
            return m_autoWrap && m_breakIterator.isBreakable(run.endOffset);
        }
    ...
    using RootLineBoxList = std::vector<std::unique_ptr<RootLineBox>>;

    class LineBuilder {
    public:
        LineBuilder(BlockFlowBox* block, FragmentBuilder* fragmentainer,
                    RootLineBoxList& lines);
