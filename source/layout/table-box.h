...
namespace plutobook {
    ...
    public:
        static constexpr ClassKind classKind = ClassKind::Table;
        ...
        void computePreferredWidths(float& minPreferredWidth,
                                    float& maxPreferredWidth) const final;
    ...
    public:
        static std::unique_ptr<FixedTableLayoutAlgorithm>
        create(TableBox* table);
    ...
    public:
        static std::unique_ptr<AutoTableLayoutAlgorithm>
        create(TableBox* table);
    ...
    public:
        static constexpr ClassKind classKind = ClassKind::TableSection;
    ...
    public:
        static constexpr ClassKind classKind = ClassKind::TableRow;
    ...
    class TableColumn {
    public:
        explicit TableColumn(TableColumnBox* box) : m_box(box) {}
    ...
    inline size_t TableBox::columnCount() const { return m_columns.size(); }

    class TableColumnBox final : public Box {
    public:
        static constexpr ClassKind classKind = ClassKind::TableColumn;
    ...
    public:
        ...
        TableCollapsedBorderEdge(TableCollapsedBorderSource source,
                                 LineStyle style, float width,
                                 const Color& color)
        ...
        bool exists() const {
            return m_source != TableCollapsedBorderSource::None;
        }
    ...
    inline bool operator==(const TableCollapsedBorderEdge& a,
                           const TableCollapsedBorderEdge& b) {
    ...
    public:
        ...
        const TableCollapsedBorderEdge& bottomEdge() const {
            return m_bottomEdge;
        }
        const TableCollapsedBorderEdge& leftEdge() const { return m_leftEdge; }
        const TableCollapsedBorderEdge& rightEdge() const {
            return m_rightEdge;
        }

        static TableCollapsedBorderEdge
        chooseEdge(const TableCollapsedBorderEdge& a,
                   const TableCollapsedBorderEdge& b);

        static TableCollapsedBorderEdge
        getTopEdge(TableCollapsedBorderSource source, const BoxStyle* style);
        static TableCollapsedBorderEdge
        getBottomEdge(TableCollapsedBorderSource source, const BoxStyle* style);
        static TableCollapsedBorderEdge
        getLeftEdge(TableCollapsedBorderSource source, const BoxStyle* style);
        static TableCollapsedBorderEdge
        getRightEdge(TableCollapsedBorderSource source, const BoxStyle* style);

        static TableCollapsedBorderEdge
        calcTopEdge(const TableCellBox* cellBox);
        static TableCollapsedBorderEdge
        calcBottomEdge(const TableCellBox* cellBox);
        static TableCollapsedBorderEdge
        calcLeftEdge(const TableCellBox* cellBox);
        static TableCollapsedBorderEdge
        calcRightEdge(const TableCellBox* cellBox);
    ...
    public:
        static constexpr ClassKind classKind = ClassKind::TableCell;
        ...
        void setColumnIndex(uint32_t columnIndex) {
        ...
        void paintBackgroundBehindCell(const PaintInfo& info,
                                       const Point& offset,
                                       const BoxStyle* backgroundStyle) const;
        void paintCollapsedBorders(
            const PaintInfo& info, const Point& offset,
            const TableCollapsedBorderEdge& currentEdge) const;
        void paintDecorations(const PaintInfo& info, const Point& offset) final;
    ...
    private:
        mutable std::unique_ptr<TableCollapsedBorderEdges>
            m_collapsedBorderEdges;
    ...
    public:
        static constexpr ClassKind classKind = ClassKind::TableCaption;

        TableCaptionBox(Node* node, const RefPtr<BoxStyle>& style);

        bool avoidsFloats() const final { return true; }
        float
        containingBlockWidthForContent(const BlockBox* container) const final;
