...
namespace plutobook {
    ...
    public:
        BlockBox(ClassKind type, Node* node, const RefPtr<BoxStyle>& style);

        virtual void computeIntrinsicWidths(float& minWidth,
        ...
        float startOffsetForContent() const {
            return style()->isLeftToRightDirection()
                       ? leftOffsetForContent()
                       : width() - rightOffsetForContent();
        }
        float endOffsetForContent() const {
            return style()->isRightToLeftDirection()
                       ? leftOffsetForContent()
                       : width() - rightOffsetForContent();
        }

        float availableWidthForContent() const {
            return std::max(0.f,
                            rightOffsetForContent() - leftOffsetForContent());
        }
        ...
        float shrinkWidthToAvoidFloats(float marginLeft, float marginRight,
                                       const BlockFlowBox* container) const;
        ...
        void computePositionedWidthUsing(
            const Length& widthLength, const BoxModel* container,
            Direction containerDirection, float containerWidth,
            const Length& leftLength, const Length& rightLength,
            const Length& marginLeftLength, const Length& marginRightLength,
            float& x, float& width, float& marginLeft,
            float& marginRight) const;

        void computePositionedWidth(float& x, float& width, float& marginLeft,
                                    float& marginRight) const;

        void computePositionedHeightUsing(
            const Length& heightLength, const BoxModel* container,
            float containerHeight, float contentHeight, const Length& topLength,
            const Length& bottomLength, const Length& marginTopLength,
            const Length& marginBottomLength, float& y, float& height,
            float& marginTop, float& marginBottom) const;

        void computePositionedHeight(float& y, float& height, float& marginTop,
    ...
    public:
        ...
        const FloatingBoxList* floatingBoxes() const {
            return m_floatingBoxes.get();
        }
        ...
        void addIntrudingFloats(BlockFlowBox* prevBlock, float offsetX,
                                float offsetY);
        void addOverhangingFloats(BlockFlowBox* childBlock);

        void positionFloatingBox(FloatingBox& floatingBox,
                                 FragmentBuilder* fragmentainer, float top);
        ...
        bool containsFloats() const {
            return m_floatingBoxes && !m_floatingBoxes->empty();
        }
        ...
        float leftOffsetForFloat(float top, float bottom, float offset,
                                 float* heightRemaining = nullptr) const;
        float rightOffsetForFloat(float top, float bottom, float offset,
                                  float* heightRemaining = nullptr) const;

        float leftOffsetForLine(float y, float height = 0,
                                bool indent = false) const;
        float rightOffsetForLine(float y, float height = 0,
                                 bool indent = false) const;

        float lineOffsetForAlignment(float remainingWidth) const;
        float startAlignedOffsetForLine(float y, float height = 0,
                                        bool indent = false) const;

        float startOffsetForLine(float y, float height = 0,
                                 bool indent = false) const;
        float endOffsetForLine(float y, float height = 0,
                               bool indent = false) const;

        float availableWidthForLine(float y, float height = 0,
                                    bool indent = false) const;

        void adjustFloatingBox(FragmentBuilder* fragmentainer,
                               const MarginInfo& marginInfo);
        void adjustPositionedBox(BoxFrame* child, const MarginInfo& marginInfo);

        void handleBottomOfBlock(float top, float bottom,
                                 MarginInfo& marginInfo);
        float collapseMargins(BoxFrame* child, FragmentBuilder* fragmentainer,
                              MarginInfo& marginInfo);
        ...
        float maxPositiveMarginBottom() const {
            return m_maxPositiveMarginBottom;
        }
        float maxNegativeMarginBottom() const {
        ...
        void estimateMarginTop(BoxFrame* child, float& positiveMarginTop,
                               float& negativeMarginTop) const;
        float estimateVerticalPosition(BoxFrame* child,
                                       FragmentBuilder* fragmentainer,
                                       const MarginInfo& marginInfo) const;

        float determineVerticalPosition(BoxFrame* child,
                                        FragmentBuilder* fragmentainer,
                                        MarginInfo& marginInfo);
        void determineHorizontalPosition(BoxFrame* child) const;

        float adjustBlockChildInFragmentFlow(BoxFrame* child,
                                             FragmentBuilder* fragmentainer,
                                             float top);

        void layoutBlockChild(BoxFrame* child, FragmentBuilder* fragmentainer,
                              MarginInfo& marginInfo);
        ...
        void paintContents(const PaintInfo& info, const Point& offset,
                           PaintPhase phase) override;
        void serializeChildren(OutputStream& o, int indent) const override;
    ...
    inline float BlockFlowBox::startOffsetForLine(float y, float height,
                                                  bool indent) const {
        return style()->isLeftToRightDirection()
                   ? leftOffsetForLine(y, height, indent)
                   : width() - rightOffsetForLine(y, height, indent);
    }

    inline float BlockFlowBox::endOffsetForLine(float y, float height,
                                                bool indent) const {
        return style()->isRightToLeftDirection()
                   ? leftOffsetForLine(y, height, indent)
                   : width() - rightOffsetForLine(y, height, indent);
    ...
    inline float BlockFlowBox::availableWidthForLine(float y, float height,
                                                     bool indent) const {
        return std::max(0.f, rightOffsetForLine(y, height, indent) -
                                 leftOffsetForLine(y, height, indent));
    }

    extern template bool is<BlockFlowBox>(const Box& value);
} // namespace plutobook