...
namespace plutobook {
    ...
    enum class Overflow : uint8_t { Auto, Visible, Hidden, Scroll };
    ...
    enum class ListStylePosition : uint8_t { Outside, Inside };
    ...
    enum class BackgroundBox : uint8_t { BorderBox, PaddingBox, ContentBox };

    enum class BackgroundAttachment : uint8_t { Scroll, Fixed, Local };

    enum class WritingMode : uint8_t { HorizontalTb, VerticalRl, VerticalLr };

    enum class TextOrientation : uint8_t { Mixed, Upright };

    enum class TextAnchor : uint8_t { Start, Middle, End };

    enum class TextAlign : uint8_t { Left, Center, Right, Justify, Start, End };
    ...
    constexpr bool operator&(TextDecorationLine a, TextDecorationLine b) {
    ...
    enum class FontVariantEmoji : uint8_t { Normal, Text, Emoji, Unicode };

    enum class WhiteSpace : uint8_t { Normal, Pre, Nowrap, PreLine, PreWrap };

    enum class WordBreak : uint8_t { Normal, KeepAll, BreakAll, BreakWord };

    enum class OverflowWrap : uint8_t { Normal, BreakWord, Anywhere };

    enum class Hyphens : uint8_t { Auto, None, Manual };

    enum class TableLayout : uint8_t { Auto, Fixed };

    enum class CaptionSide : uint8_t { Top, Bottom };

    enum class EmptyCells : uint8_t { Show, Hide };

    enum class BorderCollapse : uint8_t { Separate, Collapse };
    ...
    enum class BreakInside : uint8_t { Auto, Avoid, AvoidColumn, AvoidPage };

    enum class ColumnSpan : uint8_t { None, All };

    enum class ColumnFill : uint8_t { Auto, Balance };

    enum class QuoteType : uint8_t { Open, Close, NoOpen, NoClose };
    ...
    enum class MaskType : uint8_t { Luminance, Alpha };
    ...
    class Paint {
    public:
        enum class Type { None, Color, UriNone, UriColor };
    ...
    inline float Length::calc(float maximum) const {
        switch (m_type) {
        case Type::Fixed: return m_value;
        case Type::Percent: return m_value * maximum / 100.f;
    ...
    class LengthBox {
    public:
        explicit LengthBox(const Length& value)
    ...
    public:
        enum class Type { Contain, Cover, Length };
    ...
    public:
        ...
        bool isRenderable() const {
            return m_width > 0 && m_style > LineStyle::Hidden &&
                   m_color.alpha() > 0;
        }
    ...
    enum class BaselineShiftType : uint8_t { Baseline, Sub, Super, Length };

    class BaselineShift {
    public:
        BaselineShift(BaselineShiftType type,
                      const Length& length = Length::Auto)
    ...
    class VerticalAlign {
    public:
        VerticalAlign(VerticalAlignType type,
                      const Length& length = Length::Auto)
            : m_type(type), m_length(length) {}
    ...
    public:
        static RefPtr<BoxStyle> create(Node* node, PseudoType pseudoType,
                                       Display display);
        static RefPtr<BoxStyle> create(Node* node, const BoxStyle* parentStyle,
                                       PseudoType pseudoType, Display display);
        ...
        void setVerticalAlignType(VerticalAlignType verticalAlignType) {
            m_verticalAlignType = verticalAlignType;
        }
        ...
        bool isDisplayBlockType() const {
            return isDisplayBlockType(m_display);
        }
        bool isDisplayInlineType() const {
            return isDisplayInlineType(m_display);
        }
        bool isDisplayFlex() const {
            return m_display == Display::Flex ||
                   m_display == Display::InlineFlex;
        }

        bool isFloating() const {
            return m_floating == Float::Left || m_floating == Float::Right;
        }
        bool isPositioned() const {
            return m_position == Position::Absolute ||
                   m_position == Position::Fixed;
        }
        bool isLeftToRightDirection() const {
            return m_direction == Direction::Ltr;
        }
        bool isRightToLeftDirection() const {
            return m_direction == Direction::Rtl;
        }
        bool isClearLeft() const {
            return m_clear == Clear::Left || m_clear == Clear::Both;
        }
        bool isClearRight() const {
            return m_clear == Clear::Right || m_clear == Clear::Both;
        }

        bool isVerticalWritingMode() const {
            return m_writingMode != WritingMode::HorizontalTb;
        }
        bool isUprightTextOrientation() const {
            return m_textOrientation == TextOrientation::Upright;
        }
        ...
        static bool autoWrap(WhiteSpace ws) {
            return ws != WhiteSpace::Nowrap && ws != WhiteSpace::Pre;
        }
        static bool preserveNewline(WhiteSpace ws) {
            return ws != WhiteSpace::Normal && ws != WhiteSpace::Nowrap;
        }
        static bool collapseWhiteSpace(WhiteSpace ws) {
        ...
        bool breakAnywhere() const {
            return m_overflowWrap == OverflowWrap::Anywhere ||
                   m_wordBreak == WordBreak::BreakAll;
        }
        bool breakWord() const {
            return m_wordBreak == WordBreak::BreakWord ||
                   m_overflowWrap == OverflowWrap::BreakWord;
        }
        ...
        Length convertPositionComponent(CssValueID min, CssValueID max,
                                        const CssValue& value) const;
        ...
        convertVerticalAlignType(const CssValue& value);
