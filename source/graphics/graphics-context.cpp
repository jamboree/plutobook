...
void CairoGraphicsContext::fillGlyphs(FontHandle font, const GlyphRef glyphs[],
                                      unsigned glyphCount) {
    ...
    for (unsigned i = 0; i != glyphCount; ++i) {
        const auto& glyphIn = glyphs[i];
        auto& glyphOut = glyphBuffer[i];
        glyphOut.index = glyphIn.index;
        glyphOut.x = glyphIn.position.x;
        glyphOut.y = glyphIn.position.y;
    }
...
static void append_attribute(OutputStream& output, std::string_view name, std::string_view value)
{
    output << name << "='";
    for(auto cc : value) {
        if(cc == '\\' || cc == '\'')
...
void CairoGraphicsContext::addLinkAnnotation(std::string_view dest, std::string_view uri, const Rect& rect)
{
...
void CairoGraphicsContext::addLinkDestination(std::string_view name, const Point& location)
{
    if(name.empty())
        return;
    double x = location.x;
