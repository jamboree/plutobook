...
namespace plutobook {
    ...
    public:
        ...
        constexpr explicit RectOutsets(float outset)
            : RectOutsets(outset, outset, outset, outset) {}
        constexpr RectOutsets(float top, float right, float bottom, float left)
            : t(top), r(right), b(bottom), l(left) {}

        constexpr RectOutsets operator-() const {
            return RectOutsets(-t, -r, -b, -l);
        }
    ...
    public:
        ...
        constexpr Rect(const Point& origin, const Size& size)
            : Rect(origin.x, origin.y, size.w, size.h) {}
        ...
        constexpr void shrink(float t, float r, float b, float l) {
            expand(-t, -r, -b, -l);
        }

        constexpr void expand(const RectOutsets& outsets) {
            expand(outsets.t, outsets.r, outsets.b, outsets.l);
        }
        ...
        constexpr Rect translated(const Point& p) const {
        ...
        constexpr bool intersects(const Rect& r) const {
            return !isEmpty() && !r.isEmpty() && x < r.right() &&
                   r.x < right() && y < r.bottom() && r.y < bottom();
        }
    ...
    public:
        ...
        constexpr explicit RectRadii(float radius)
            : RectRadii(radius, radius) {}
        constexpr RectRadii(float rx, float ry)
            : tl(rx, ry), tr(rx, ry), bl(rx, ry), br(rx, ry) {}

        constexpr explicit RectRadii(const Size& radii)
            : RectRadii(radii, radii, radii, radii) {}
        constexpr RectRadii(const Size& tl, const Size& tr, const Size& bl,
                            const Size& br)
        ...
        constexpr bool isZero() const {
            return tl.isZero() && tr.isZero() && bl.isZero() && br.isZero();
        };
    ...
    constexpr RectRadii operator+(const RectRadii& radii,
    ...
    constexpr RectRadii& operator+=(RectRadii& radii,
                                    const RectOutsets& outsets) {
    ...
    constexpr RoundedRect operator+(const RoundedRect& rect,
                                    const RectOutsets& outsets) {
        RoundedRect r(rect);
        r.expand(outsets);
        return r;
    ...
    constexpr RoundedRect& operator+=(RoundedRect& rect,
                                      const RectOutsets& outsets) {
        rect.expand(outsets);
        return rect;
    }
